...
public static class PointsModConfig
{
	private const string ConfigDirectoryName = "Bepinex/config/ETS/Config/PointsMod";
	private const string ConfigFileName = "points_mod_config.json";
    ...
    public static PointsModConfigData Config { get; private set; } = new PointsModConfigData();
    ...
    public static void Load()
	{
		...
		try
		{
			...
			Config = JsonSerializer.Deserialize<PointsModConfigData>(jsonData, SerializerOptions) ?? new PointsModConfigData();
		}
		catch (Exception ex)
		{
			Config = new PointsModConfigData();
...
public class PointsModConfigData
{
	public bool UsePhysicalCurrency { get; set; } = false;
	public string MainVirtualCurrencyName { get; set; } = "Points";
    public string CosmeticVirtualCurrencyName { get; set; } = "Ichor";
}
