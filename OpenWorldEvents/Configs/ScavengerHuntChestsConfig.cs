...
public static class ScavengerHuntChestConfig
{
	private const string ConfigDirectoryName = "Bepinex/config/ETS/Config/OpenWorldEvents";
	private const string ConfigFileName = "scavenger_hunt_chest.json";
	private static readonly string FullPath = Path.Combine(ConfigDirectoryName, ConfigFileName);
    private static JsonSerializerOptions SerializerOptions = new()
    {
        WriteIndented = true
    };
    public static ScavengerHuntChestConfigData Config { get; private set; } = new ScavengerHuntChestConfigData();
    ...
    public static void Load()
	{
		...
		try
		{
			...
			Config = JsonSerializer.Deserialize<ScavengerHuntChestConfigData>(jsonData, SerializerOptions) ?? new ScavengerHuntChestConfigData();
		}
		catch (Exception ex)
		{
			Config = new ScavengerHuntChestConfigData();
...
public class ScavengerHuntChestConfigData
{
    public int ChestDelaySeconds { get; set; } = 300;
    public Dictionary<string, ScavengerChestData> ScavengerHuntChestLoot { get; set; } = new()
    {
        { "fishchest", new() }
    };
    public class ScavengerChestData
    {
        public string ChestName { get; set; } = "Fish Chest";
        public bool Greater { get; set; } = true;
