...
public static class HorseConfig
{
	...
	private const string ConfigFileName = "horse_config.json";
	private static readonly string FullPath = Path.Combine(ConfigDirectoryName, ConfigFileName);
    private static JsonSerializerOptions SerializerOptions = new()
    {
        WriteIndented = true
    };
    public static HorseConfigData Config { get; private set; } = new HorseConfigData();
    ...
    public static void Load()
	{
		...
		try
		{
			...
			Config = JsonSerializer.Deserialize<HorseConfigData>(jsonData, SerializerOptions) ?? new HorseConfigData();
		}
		catch (Exception ex)
		{
			Config = new HorseConfigData();
...
{
    public bool CursedFogOnRider { get; set; } = true;
    public bool AnnounceEvent { get; set; } = true;
    public bool DisableDurabilityLossDuringEvent { get; set; } = false;
    public float HorseSpeed { get; set; } = 6.2f;
    public float HorseAcceleration { get; set; } = 2;
    public int HorseDelaySeconds { get; set; } = 300;
    public List<TimeOnly> HorseSpawnTimes { get; set; } = new()
    {
        new TimeOnly(14, 1),
        new TimeOnly(20, 1),
        new TimeOnly(2, 1),
    ...
    public List<CoordinateDto> HorseSpawnLocations { get; set; } = new()
    {
        new CoordinateDto
        {
            X = -1000f,
    ...
    public List<ItemDTO> HorseItems { get; set; } = new()
    {
        new ItemDTO()
    };
}
